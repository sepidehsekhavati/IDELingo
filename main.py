...
class IDELingoApp:
    def __init__(self):
        ...
        # متغیرهای مورد نیاز برای Vocabulary و Grammar
        self.vocab_diff = None
        self.vocab_container = None
        self.grammar_fav_only = True
        self.grammar_search = None
        self.grammar_container = None
        self.phrases_container = None
    ...
    def open_grammar_topic(self, topic_key):
        ...
        if not info:
            self._show_snack("Grammar rule not found!", COLORS['danger'])
            return
                ...
                note_card = ft.Card(
                    content=ft.Container(
                        content=ft.Column([
                            ...
                            display_text,
                            ft.Row([
                                ft.TextButton("✏️ Edit", on_click=lambda e, t=topic_key, old=text, ts=timestamp: self._edit_note_dialog(t, old, ts, refresh_notes)),
                                ft.TextButton("🗑️ Delete", on_click=lambda e, t=topic_key, old=text, ts=timestamp: self._delete_note(t, old, ts, refresh_notes), icon_color=COLORS['danger'])
    ...
    def _edit_note_dialog(self, topic_key, old_text, timestamp, refresh_callback):
        ...
        edit_dialog = ft.AlertDialog(
            title=ft.Text("Edit Note", color=COLORS['accent']),
            content=ft.Container(content=edit_field, padding=20),
            actions=[
                ft.TextButton("Cancel", on_click=lambda e: setattr(edit_dialog, 'open', False)),
                ft.ElevatedButton("Save", on_click=save_edit, bgcolor=COLORS['success'])
            ]
        )
    ...
    def _delete_note(self, topic_key, old_text, timestamp, refresh_callback):
        def confirm_delete(e):
            ...
            self._show_snack("🗑️ Note deleted!", COLORS['success'])
        confirm_dialog = ft.AlertDialog(
            title=ft.Text("Delete Note", color=COLORS['warning']),
            ...
            actions=[
                ...
                ft.ElevatedButton("Delete", on_click=confirm_delete, bgcolor=COLORS['danger'])
            ]
        )
    ...
    def show_community(self):
        ...
        def search(e):
            ...
            if not users:
                ...
                for u in users:
                    res.controls.append(ft.Container(content=ft.Row([
                        ft.Text(u[2], size=32),
                        ft.Column([ft.Text(u[1], size=14, weight=ft.FontWeight.BOLD, color=COLORS['text']), ft.Text(f"Level {u[3]} • {u[4]} XP", size=11, color=COLORS['text_secondary'])], spacing=3, expand=True),
                        ft.ElevatedButton("View Profile", on_click=lambda _, uid=u[0]: self.view_profile(uid), bgcolor=COLORS['info'], height=35)
                    ]), bgcolor=COLORS['card'], border_radius=10, padding=12))
            self.page.update()
        sf.on_submit = search
        self.page.add(ft.Column([
            ft.Container(content=ft.Text("👥 Community", size=24, weight=ft.FontWeight.BOLD, color=COLORS['accent']), padding=20),
            ft.Container(content=ft.Row([sf, ft.ElevatedButton("Search", on_click=search, bgcolor=COLORS['accent'])], spacing=10), padding=ft.padding.symmetric(horizontal=20)),
            ...
            ft.Container(content=res, expand=True, padding=20),
            self._bottom_nav_bar()
        ], spacing=10, expand=True))
        self.page.update()
    ...
    def view_profile(self, uid):
        ...
        if err:
            ...
            return
        dlg = ft.AlertDialog(title=ft.Text(f"{prof['username']}'s Profile", color=COLORS['accent']), content=ft.Container(content=ft.Column([
            ft.Container(content=ft.Text(prof['avatar'], size=50), alignment=ft.alignment.center),
            ft.Text(prof['username'], size=20, weight=ft.FontWeight.BOLD, color=COLORS['text']),
            ft.Divider(),
            ft.Row([
                ft.Column([ft.Text("⭐ Level", size=12), ft.Text(str(prof['level']), size=18, weight=ft.FontWeight.BOLD, color=COLORS['accent'])], horizontal_alignment=ft.CrossAxisAlignment.CENTER, expand=True),
    ...
    def show_leaderboard(self):
        ...
        col.controls.append(ft.Container(content=ft.Row([
        ...
        ]), padding=10, bgcolor=COLORS['sidebar'], border_radius=8))
        for i, (name, av, lvl, score) in enumerate(lb, 1):
            medal = "🥇" if i==1 else "🥈" if i==2 else "🥉" if i==3 else f"{i}."
            is_me = name == self.current_user['username']
            col.controls.append(ft.Container(content=ft.Row([
                ft.Text(medal, width=50),
                ft.Text(f"{av} {name}", width=150, expand=True, weight=ft.FontWeight.BOLD if is_me else None, color=COLORS['accent'] if is_me else COLORS['text']),
                ft.Text(f"Lv.{lvl}", width=60),
                ft.Text(str(score), width=100, color=COLORS['success'] if i<=3 else COLORS['text_secondary'])
            ]), bgcolor=COLORS['accent'] if is_me else COLORS['card'], border_radius=8, padding=10))
        self.page.add(ft.Column([
            ft.Container(content=ft.Text("🏆 Leaderboard", size=24, weight=ft.FontWeight.BOLD, color=COLORS['accent']), padding=20),
            ...
            ft.Container(content=col, expand=True, padding=20),
            self._bottom_nav_bar()
        ], spacing=10, expand=True))
        self.page.update()
    ...
    def show_settings(self):
        ...
        plan_type = ft.Dropdown(label="Plan Type", options=[ft.dropdown.Option("daily"), ft.dropdown.Option("weekly"), ft.dropdown.Option("monthly"), ft.dropdown.Option("custom")],
            value=plan.get('plan_type','daily'), width=200)
        ...
        def delete_data(e):
            def confirm(e):
                ...
                for table in ['vocabulary','phrases','practice_chat','daily_progress']:
                    self.user_manager.db.execute_query(f"DELETE FROM {table} WHERE user_id=?", (self.current_user['id'],), commit=True)
                self._show_snack("✅ All learning data cleared!", COLORS['success'])
                self.show_dashboard()
            confirm_dlg = ft.AlertDialog(title=ft.Text("Clear Data", color=COLORS['warning']), content=ft.Text("This will delete all your vocabulary, phrases, and practice history. This cannot be undone!"),
                actions=[ft.TextButton("Cancel", on_click=lambda e: self._close_dialog(confirm_dlg)),
        ...
        tabs = ft.Tabs(selected_index=0, tabs=[
            ft.Tab(text="⚙️ General", content=ft.Container(content=ft.Column([
                ...
                ft.Divider(),
                ft.Text("Avatar", size=14, weight=ft.FontWeight.BOLD), avatar_row
            ], spacing=15, scroll=ft.ScrollMode.AUTO), padding=20, expand=True)),
            ft.Tab(text="📅 Learning Plan", content=ft.Container(content=ft.Column([
                ft.Text("Personalized Learning Plan", size=18, weight=ft.FontWeight.BOLD, color=COLORS['accent']),
                plan_type, goals_col, ft.ElevatedButton("💾 Save Plan Settings", on_click=save_plan, bgcolor=COLORS['success']),
                ft.Divider(),
                ft.Text(f"🔥 Current Streak: {plan.get('current_streak',0)} days", size=13),
            ...
            ft.Tab(text="🔒 Privacy", content=ft.Container(content=ft.Column([
                ...
                ft.Divider(),
                ft.ElevatedButton("Delete Account Data", on_click=delete_data, bgcolor=COLORS['danger']),
